...
def step_1_validate_paths(state):
    ...
    # 常规的非空校验（站在字符串的角度）
    if not pdf_path:
        logger.error("step_1_validate_paths检查发现没有输入文件，无法解析！")
        raise ValueError("sstep_1_validate_paths检查发现没有输入文件，无法解析！")
    if not local_dir:
        # 给予一个输出的默认值
        local_dir = str(PROJECT_ROOT / "output")
        logger.info(f"step_1_validate_paths检查发现local_dir没有赋值，请给予默认值：{local_dir}")
    # 进行文件存在校验
    ...
    if not local_dir_obj.exists():
        logger.error("step_1_validate_paths检查发现local_dir不存在，请主动创建对应的文件夹！")
...
def step_2_upload_and_poll(pdf_path_obj)->str:
    ...
    uploaded_url = response.json()['data']['file_urls'][0] # 上传文件地址
    batch_id = response.json()['data']['batch_id'] # 处理id ，后续根据这个获取结果！

    # 2.将文件上传到对应的解析地址
    # 使用Put请求，将pdf_path_obj文件传递到uploaded_url地址即可
    # 注意：不能直接使用put!大概率报错！原因：电脑开了代理，put的请求头，添加一些额外的参数头，将文件真的转存到第三方的文件存储服务器！
    # 文件存储服务器检查都比较严格！ 拒绝存储，报错，get post 款进宽出 put严进严出
    ...
    # 3. 轮询获取解析结果
    # 循环获取！确保获取到结果，再先后执行
    # 设计一个循环，3秒获取一次！最多等待10min(600s) -> 600页pdf
    ...
    while True:
        ...
        if res.status_code != 200:
            # 5xx系列我们会给予机会，直到timeout
            # http的状态码 1xx 2xx 3xx 4xx [5xx]
            if 500 <= res.status_code < 600:
                time.sleep(poll_interval)
                continue
            raise RuntimeError(f"[step_2_upload_and_poll] 请求minerU解析接口失败，返回的状态码是{res.status_code}！")

        json_data = res.json() # 获取本次结果
        if json_data['code'] !=0:
            # ! = 0 很大概率token过期了
            raise RuntimeError(f"[step_2_upload_and_poll] 请求minerU解析接口失败，返回的错误是{json_data['code']}信息{json_data['msg']}！")
...
def step_3_download_and_extract(zip_url, local_dir_obj, stem):
    ...
    http_session = requests.Session()
    http_session.trust_env = False  # 禁止走代理，避免代理干扰 SSL 握手
    response = http_session.get(zip_url, verify=False)
    ...
    # 3.清空旧目录（将上一次处理的文件目录进行删除）
    extract_target_dir = local_dir_obj / stem
    # 先清空文件内容，因为两次解压的文件数量可能不一样，会保留数据
    ...
    # 5.返回md文件的地址
    # 解压以后得文件的文件名 可能叫 文件.md ，低版本也可能叫 full.md
    ...
    # md文件名 二狗子.md full.md 不确定.md
    # 统一改成 原文件名（stem）.md
    # 不是原名字，重命名
    if target_md_file.stem != stem:
        # 进行重命名
        # target_md_file.with_name(f"{stem}.mdm") 修改path对象（不涉及文件操作）返回结果是修改后path对象
        # target_md_file.rename(target_md_file.with_name(f"{stem}.md")) 修改磁盘中的文件名称， return 新的路径path
...
def node_pdf_to_md(state: ImportGraphState) -> ImportGraphState:
    ...
    未来要实现:
    1. 进入的日志和任务状态的配置
    2. 进行参数校验（local_dir -> 给与默认值 ｜ local_file_path完成字面意思的校验 -> 深入校验，校验的文件是否真的存在）
    3. 调用minerU进行pdf的解析（local_file_path）返回一个下载文件地址 xx.zip url地址
    ...
    logger.info(f">>> [{function_name}]开始执行了！现在的状体为：{state}")
    add_running_task(state['task_id'], function_name)
    try:
        # 2.进行参数校验（local_dir -> 给予默认值 ｜ local_file_path完成字面意思的校验 -> 深入校验，校验文件是否真的存在）
        ...
        # 4.下载zip包，并且解析提取（local_dir）
        # 参数：1.要下载的地址 2.local_dir_obj 解压的文件 3.文件名 二狗子（二狗子.pdf）
        # 返回值：解压后md文件的真实路径
        ...
        # 处理异常
        logger.info(f">>> [{function_name}]使用minerU解析发生了异常，异常信息：{e}")
        raise # 终止工作流
    finally:
        # 6.结束的日志和任务状态的配置
        logger.info(f">>> [{function_name}]结束了！现在的状体为：{state}")
...
if __name__ == "__main__":
    ...
    logger.info("===== 结束node_pdf_to_md 节点单元测试 =====")